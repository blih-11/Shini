      ...
      <div className="relative h-[42vh] min-h-[280px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center px-4 sm:px-6">
          <div className="max-w-7xl w-full mx-auto">
            <span className="text-brand-accent text-xs font-semibold uppercase tracking-widest">Locations</span>
            <h1 className="text-brand-cream text-4xl md:text-5xl font-black mt-2">Find a Store</h1>
            <p className="text-brand-cream/65 mt-2 max-w-md text-sm">Visit us in person for a curated shopping experience and personalized styling.</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="grid md:grid-cols-3 gap-5 mb-14">
          {storeLocations.map((store, i) => (
            <div key={store.id} className="bg-white/4 border border-white/8 hover:border-brand-accent/35 transition-all duration-200 rounded-2xl p-6 group">
              <div className="flex items-start justify-between mb-4">
                <span className="text-brand-accent text-2xl font-black">{String(i + 1).padStart(2, '0')}</span>
                <div className="w-8 h-8 rounded-full bg-brand-accent/10 flex items-center justify-center text-brand-accent group-hover:bg-brand-accent group-hover:text-white transition-all duration-200">
              ...
              <div className="space-y-2.5 text-sm">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <a href={`tel:${store.phone}`} className="text-brand-muted hover:text-brand-accent transition-colors text-xs">{store.phone}</a>
                </div>
                <div className="flex items-start gap-2.5">
              ...
              <a href={store.mapUrl} target="_blank" rel="noreferrer" className="mt-5 flex items-center gap-1.5 text-brand-accent text-xs font-semibold hover:gap-2.5 transition-all">
        ...
        <div className="rounded-2xl overflow-hidden border border-white/8 mb-14 bg-white/4 aspect-[16/6] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-brand-muted text-xs mt-1">Use "Get Directions" for navigation</p>
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-5">
          {[
            { icon: '✦', title: 'Personal Styling', desc: 'Book a one-on-one session with our in-store stylists for a tailored shopping experience.' },
            { icon: '◈', title: 'Try Before You Buy', desc: 'Experience the full range in person. Touch the fabrics, try the fit, feel the quality.' },
            { icon: '◇', title: 'In-Store Exclusives', desc: 'Store-only promotions and early access to new collections available only for walk-in shoppers.' },
          ].map(item => (
            <div key={item.title} className="text-center p-6 bg-white/4 rounded-2xl border border-white/8">
              <div className="text-brand-accent text-2xl mb-3">{item.icon}</div>
