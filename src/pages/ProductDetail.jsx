...
export default function ProductDetail() {
  ...
  if (!product) return (
    <div className="min-h-screen bg-brand-bg flex flex-col items-center justify-center gap-4 text-center px-4">
      ...
      <button onClick={() => navigate('/products')} className="text-brand-accent text-sm hover:underline">Browse Products</button>
    </div>
  );
        ...
        <div className="grid lg:grid-cols-2 gap-10 xl:gap-14">
          ...
          <div className="space-y-3">
            ...
            {images.length > 1 && (
              <div className="flex gap-3">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setSelectedImg(i)} className={`w-20 h-24 rounded-lg overflow-hidden border-2 transition-all ${selectedImg === i ? 'border-brand-accent' : 'border-white/10 hover:border-white/30'}`}>
          ...
          <div>
            <div className="flex flex-wrap gap-2 mb-3">
              {product.isNew && <span className="bg-brand-accent/15 text-brand-accent border border-brand-accent/25 text-[10px] font-bold px-3 py-1 rounded-md uppercase tracking-wide">New Arrival</span>}
            ...
            <div className="flex items-baseline gap-3 mb-2">
              ...
              {discount && <span className="text-brand-accent text-sm font-bold">Save ${(product.originalPrice - product.price).toFixed(2)}</span>}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {product.colors.map(color => (
                  <button key={color} onClick={() => { setSelectedColor(color); setColorError(false); }}
                    className={`px-4 py-2 rounded-lg border text-sm transition-all duration-200 ${selectedColor === color ? 'border-brand-accent bg-brand-accent/10 text-brand-accent' : 'border-white/15 text-brand-muted hover:border-white/35 hover:text-brand-cream'}`}>
                    {color}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {product.sizes.map(size => (
                  <button key={size} onClick={() => { setSelectedSize(size); setSizeError(false); }}
                    className={`min-w-[50px] px-3 py-2 rounded-lg border text-sm transition-all duration-200 font-medium ${selectedSize === size ? 'border-brand-accent bg-brand-accent/10 text-brand-accent' : 'border-white/15 text-brand-muted hover:border-white/35 hover:text-brand-cream'}`}>
                    {size}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3 mb-7">
              <button onClick={handleAddToCart} disabled={!product.inStock}
                className="flex-1 bg-brand-accent text-white font-semibold py-3.5 px-6 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed text-sm">
                {product.inStock ? 'Add to Cart' : 'Out of Stock'}
              </button>
              <button onClick={() => setCartOpen(true)} className="px-6 py-3.5 border border-white/15 text-brand-cream rounded-xl hover:border-brand-accent hover:text-brand-accent transition-all text-sm font-medium">
                View Cart
              </button>
            </div>
