...
export default function Contact() {
  ...
  const contactItems = [
    { icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>, label: 'Email Us', value: 'hello@shini.style', href: 'mailto:hello@shini.style' },
  ...
  return (
    <div className="bg-brand-bg min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="mb-8">
          <span className="text-brand-accent text-xs font-semibold uppercase tracking-widest">Get in Touch</span>
          <h1 className="text-brand-cream text-3xl md:text-4xl font-bold mt-1 mb-2">Contact Us</h1>
          <p className="text-brand-muted text-sm max-w-lg">Have a question, a styling query, or just want to say hi? We'd love to hear from you.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-10">
          <div className="bg-white/3 border border-white/8 rounded-2xl p-7">
            {submitted ? (
              <div className="flex flex-col items-center justify-center h-full py-14 text-center gap-4">
                <div className="w-14 h-14 bg-green-500/15 rounded-full flex items-center justify-center text-3xl">✓</div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {[['name', 'Name', 'Your full name', 'text'], ['email', 'Email', 'your@email.com', 'email']].map(([name, label, ph, type]) => (
                    <div key={name}>
                      ...
                      <input name={name} type={type} value={formData[name]} onChange={e => setFormData(p => ({ ...p, [e.target.name]: e.target.value }))} required placeholder={ph}
                        className="w-full bg-white/5 border border-white/12 text-brand-cream placeholder:text-brand-muted text-sm px-4 py-3 rounded-xl focus:outline-none focus:border-brand-accent transition-colors" />
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <select name="subject" value={formData.subject} onChange={e => setFormData(p => ({ ...p, subject: e.target.value }))} required
                    className="w-full bg-white/5 border border-white/12 text-brand-muted text-sm px-4 py-3 rounded-xl focus:outline-none focus:border-brand-accent transition-colors appearance-none cursor-pointer">
                ...
                <div>
                  ...
                  <textarea name="message" value={formData.message} onChange={e => setFormData(p => ({ ...p, message: e.target.value }))} required rows={5} placeholder="Tell us how we can help..."
                    className="w-full bg-white/5 border border-white/12 text-brand-cream placeholder:text-brand-muted text-sm px-4 py-3 rounded-xl focus:outline-none focus:border-brand-accent transition-colors resize-none" />
                </div>
                <button type="submit" className="w-full bg-brand-accent text-white font-semibold py-3.5 rounded-xl hover:opacity-90 transition-opacity">Send Message</button>
              </form>
            )}
          </div>
          ...
          <div className="space-y-5">
              ...
              <div className="space-y-3">
                {contactItems.map(item => (
                  <a key={item.label} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer"
                    className="flex items-center gap-4 p-4 bg-white/4 border border-white/8 rounded-xl hover:border-brand-accent/35 transition-all group">
                    <div className="w-10 h-10 bg-brand-accent/10 rounded-full flex items-center justify-center text-brand-accent group-hover:bg-brand-accent group-hover:text-white transition-all">{item.icon}</div>
                    <div>
            ...
            <div className="bg-brand-accent/10 border border-brand-accent/20 rounded-xl p-5">
              ...
              <p className="text-brand-muted text-sm leading-relaxed">We typically respond within 24 hours on business days. For urgent queries, reach us on WhatsApp for faster response.</p>
            </div>
